...
2. Compute the cycle boundaries by adding calendar months (not fixed 30-day blocks — "14/07 → 14/08" is same day next month, which varies between 28–31 days) to the start date. **This math is new — there is no existing calendar-month logic in the script to reuse** (the 7-day label at `statusline.sh:507` is plain epoch ceiling-division). Add months by incrementing the month/year fields numerically and re-parsing via the same dual-command `date` pattern as 5d; if the start day doesn't exist in the target month (e.g. 31/01 → February), clamp to that month's last day. **Superseded (v2.4.1):** this originally rolled forward through months until `cycle_end > now`, which snapped the bar back to 0% at each renewal boundary. Renewal isn't automatic, so it now tracks a *single* cycle (`start_date .. start_date + 1 month`) and pins at 100% once elapsed, until `/super-status:subscribe` bumps the date.
